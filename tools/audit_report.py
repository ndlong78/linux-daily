...
def build(*, as_of: date, github: bool = False, production: bool = False) -> tuple[str, int]:
    ...
    failures = len(health.errors) + len(mix_errors) + len(workflow_failures)
    ...
    for key in ("posts", "generated_pages", "technical_sources", "social_code_images", "woff2_fonts", "rss_items", "sitemap_urls"):
        lines.append(f"| `{key}` | {health.metrics.get(key, 0)} |")

    lines.extend(["", "## Workflow evidence", "", "| Workflow | Status | SHA |", "|---|---|---|"])
    for state in workflows:
        lines.append(f"| {state.name} | **{_workflow_badge(state)}** | `{state.head_sha[:8] if state.head_sha else 'n/a'}` |")
    ...
    if production_result is None:
        ...
        lines.append(f"- Expected fingerprint: `{production_result.expected_fingerprint or 'n/a'}`")
        lines.append(f"- Production fingerprint: `{production_result.production_fingerprint or 'n/a'}`")
    ...
    if findings:
        ...
        lines.append("- PASS: no audit findings in the requested evidence set.")

    lines.extend([
        "",
        "> This report is derived evidence only. Repository metadata/artifacts, GitHub Actions and the production serving state remain the sources of truth.",
        "",
    ])
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--github", action="store_true", help="Include latest main CI/Production Smoke evidence.")
    parser.add_argument("--production", action="store_true", help="Run live production observability once.")
